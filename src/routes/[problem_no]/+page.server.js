...
async function loadProblemInfo(pno) {
	return await fetch(`http://code-study-api.kr-1-ts.esukmean.com/problem/${pno}`).then((f) => f.json());
}
...
export const actions = {
	default: async ({ params, request, cookies }) => {
		...
		await fetch(`http://code-study-api.kr-1-ts.esukmean.com/problem/${problem_no}/`, {
			method: 'POST',
			headers: {
